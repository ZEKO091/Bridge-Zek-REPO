...
if (!window.electronAPI) {
  const mock: ElectronAPI = {
    ...
    wsSave: async () => true,
    wsLoad: async () => null,
    authSignup: async () => { try { const r = await fetch('http://localhost:6060/api/signup', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:arguments[0],email:arguments[1],password:arguments[2]})}); return await r.json() } catch { return {ok:false,data:{error:'Auth server not running. Start: node server/server.js'}} } },
    authLogin: async () => { try { const r = await fetch('http://localhost:6060/api/login', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:arguments[0],password:arguments[1]})}); return await r.json() } catch { return {ok:false,data:{error:'Auth server not running. Start: node server/server.js'}} } },
    authVerify: async () => { try { const r = await fetch('http://localhost:6060/api/verify-session', {method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${arguments[0]}`}}); return await r.json() } catch { return {ok:false,data:{error:'Auth server not running'}} } },
    wsDelete: async () => true,
    onVoiceLog: () => () => {},
    onVoiceStopped: () => () => {},
    checkUpdate: async () => {},
