...
if (!window.electronAPI) {
  const mock: ElectronAPI = {
    ...
    voiceStart: async () => { try { const r = await fetch('/api/voice/start'); if (r.ok) return (await r.json()).ok; return false } catch { return false } },
    voiceStop: async () => { try { await fetch('/api/voice/stop'); return true } catch { return true } },
    voiceStatus: async () => { try { const r = await fetch('/api/voice/status'); if (r.ok) return (await r.json()).running; return false } catch { return false } },
    wsSave: async () => true,
    wsLoad: async () => null,
    wsDelete: async () => true,
    onVoiceLog: () => () => {},
